...
function parseLicenseTier(value: FormDataEntryValue | null): LicenseTier {
  if (
    value &&
    LICENSE_TIERS.includes(value as (typeof LICENSE_TIERS)[number])
  ) {
...
function parseLicenseStatus(
  ...
  if (
    value &&
    LICENSE_STATUSES.includes(value as (typeof LICENSE_STATUSES)[number])
  ) {
...
function parseRole(value: FormDataEntryValue | null): OrgRole {
  if (
    value &&
    ORG_ROLES.includes(value as (typeof ORG_ROLES)[number])
  ) {
...
export async function profileUpdateIndividualLicenseAction(
  formData: FormData
) {
  ...
  try {
    ...
    redirectUrl = "/app/profile?notice=individual_license_updated";
  } catch (error) {
    safeRedirect("/app/profile", error);
  }

  redirect(redirectUrl!);
}

export async function profileUpdateOrgLicenseAction(formData: FormData) {
  const session = await requireSession();
  const auth = buildApiAuthContext(session);
  const apiBaseUrl = readApiBaseUrl();
  ...
  try {
    ...
    redirectUrl = `/app/profile?org=${orgId}&notice=organization_license_updated`;
  } catch (error) {
    safeRedirect("/app/profile", error);
  }
...
export async function profileUpdateOrgMemberRoleAction(
  formData: FormData
) {
  const session = await requireSession();
  const auth = buildApiAuthContext(session);
  ...
  try {
    const orgId = parseIntField(formData.get("orgId"), "orgId");
    const memberUserId = parseIntField(
      formData.get("memberUserId"),
      "memberUserId"
    );
    ...
    const actingMember = members.members.find(
      (m) => m.user_id === me.user_id
    );
    if (!actingMember) {
    ...
    if (isSelf) {
      ...
      if (newRank < currentRank) {
        throw new Error(
          `Cannot lower your own role from ${actingMember.role} to ${newRole}. ` +
            "Ask another owner or admin to change your role."
        );
      }
    }
    ...
    redirectUrl = `/app/profile?org=${orgId}&notice=member_role_updated`;
  } catch (error) {
    safeRedirect("/app/profile", error);
  }
...
export async function profileCreateOrgAction(formData: FormData) {
  const session = await requireSession();
  const auth = buildApiAuthContext(session);
  const apiBaseUrl = readApiBaseUrl();
  ...
  if (!name) {
    redirect("/app/profile?error=organization_name_required");
  }

  let redirectUrl: string;
  try {
    const created = await createOrg(auth, { name }, apiBaseUrl);
    redirectUrl = `/app/profile?org=${created.org_id}&notice=organization_created`;
  } catch (error) {
    safeRedirect("/app/profile", error);
  }
